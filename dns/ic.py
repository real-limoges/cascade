...
def model_spectrum(k, E0, kp, kc=None):
    """E(k) ~ k^4 exp(-2 (k/kp)^2), normalised so sum E(k) = E0 over the
    retained shells (k <= kc if given)."""
...
def isotropic_field(grid: SpectralGrid, E0=1.5, kp=3.0, seed=12345):
    ...
    Ek_tar = model_spectrum(k, E0, kp, kc=grid.kc)
